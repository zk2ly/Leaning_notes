...
71  用stringstream和getline实现split字符串 然后判断分割后的每部分 如果是空 一个点或者 两个点但是队列为空时 都跳过 遇到字符串就入队 两个点就队尾元素出队 最后判断队列是否为空 如果为空返回"/" 否则依次出队 并组合成字符串
...
三者都可以用递归，不多介绍。非递归方法要用到栈，对于当前要访问的节点cur，如果cur非空或者栈非空的时候，首先循环把cur和cur->left都入栈。对于前序遍历，入栈的同时进行访问，因此栈中保存的节点只有右子树没有访问，栈中元素依次出栈并作为新的cur进行访问;对于中序遍历，入栈的同时不做访问，因此栈中保存的是自身和右节点没有访问的节点，栈中元素依次出栈的时候首先访问该元素，然后再作为新的cur进行访问；对于后序遍历，入栈的时候同样不访问，因此栈中保存的是自身和右节点没有访问的节点，但是后序遍历要先访问右节点，再会过来访问根节点，所以要多一个last指针指向上一次访问的元素，依次取栈顶元素，如果栈顶元素没有右节点或者右节点就是上一次访问过的节点，此时访问这个节点并且出栈，标记这次访问的节点，cur置为null, 否则访问栈顶元素的右节点。这里cur置为空是因为栈顶元素已经出栈，下一次要访问的节点应该是新的栈顶元素。

## 5.3 队列的典型应用
102 层序遍历 : 和BFS有所不同 BFS返回的是一个一维数组 因此只需要先将根节点入队 然后当队列不为空时 循环的访问队首元素 并且把该元素左右节点入队即可；但是层序遍历返回的是一个二维数组，要求把不同层的节点放到一个数组中，我们知道对于当前层的所有节点，它的左右节点都是下一层的节点，且同一层的节点他们的左右节点也都属于下一层，所以我们每次要记录队中元素数量size，把size个元素都出队到一个一维数组中，此时队中保存的就是他们下一层的节点，循环记录节点个数并出队到下一个一维数组，依次执行。

103 锯齿形层序遍历 : 层序遍历时我们可以从左至右的拿到每层的节点，这里需要用一个flag来表示是否是从左至右的访问这些节点，如果是那么正常访问即可，如果不是就需要反过来访问，因此有正有反可以用双端队列deque，如果是从左至右访问，就把queue中依次出队的元素在deque中依次push_back入队，如果是从右至左访问，就把queue中依次出队的元素在deque中依次push_front入队，然后用vector<int>(dq.begin(),dq.end())来创建这一层的数组并加入到最终数组res中即可。

107 层序遍历: 最后用reverse反转res即可 
...
# 6.二叉树和递归
## 6.1 二叉树天然的递归结构
104 最大深度 递归调用函数求得左右子树的高度 选大的加一即为整棵树的高度 注意递归边界：当根节点不存在时 返回0
...
## 6.2 简单的二叉树问题
226 反转二叉树 递归边界：1.空节点返回空  2. 叶子节点直接返回  递归逻辑：根节点的左指针指向记录的已经反转的右子树  右指针指向记录的已经反转的左子树 最后返回根节点

100 判断两个树是否相等 递归边界：1.都为空相等 2.只有一个不为空不相等 3.两个都不为空但是值不一致时不相等  递归逻辑： 左子树和右子树都相等时 才相等

101 如果一个树是镜像的 那么这棵树是对称的 即左右子树是对称的 因此问题转换为两棵树什么时候是对称的 递归边界：1.两棵树都为空 对称  2.只有一个为空 不对称 3. 都不为空但是值不相等 不对称  递归逻辑：p的左子树和q的右子树对称   p的右子树和q的左子树对称 才能说p和q两棵树对称

222 求节点个数 如果为空 返回0 否则递归调用左右子树节点和+1

110 求一棵树是不是平衡二叉树：每一个节点的左右子树相差不大于1  根节点的左右子树相差不大于1 且根节点的左右子树各自的左右子树相差也不大与1

## 6.3 注意递归的终止条件(叶子节点)
112 根节点到叶子节点是否有一条路径 使得路径上所有点之和等于target？  递归边界：1.如果为空 返回fasle  2.如果没有左右子树 查看值是否与target相等  递归逻辑：对于任意非叶子节点 每经过一个节点 用target减去当前节点的值 然后再到左右子树中找是否有符合要求的叶子节点 只要有一个就返回true 否则返回false
