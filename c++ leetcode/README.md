...
```
## 4.1 反转链表
206 令当前的结点cur指向前一个结点pre的时候 会导致后一个结点丢失 因此每次在指之前 要用nex记录后一个结点 指完之后 cur和pre都后移 直到cur等于空 说明已经全部返回完了 

92 链表分为三部分 A->B->C 先翻转B 然后再拼接 因此要记录A的最后一个结点 和 B的最后一个结点 B翻转后的最后一个结点就是此时B的第一个结点 即第m个结点  这里为了防止A不存在 没有next指针 因此要设置虚拟头节点 C可以不存在
