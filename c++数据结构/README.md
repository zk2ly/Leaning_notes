...
- [3.图型数据结构](#3图型数据结构)
  ...
  - [3.3 单源最短路径](#33-单源最短路径)
    - [3.3.1 dijkstra算法](#331-dijkstra算法)
    - [3.3.2 bellman ford算法](#332-bellman-ford算法)
...
没有负权环的情况下 一个顶点到另一个顶点的最短路径 最多经过v-1条边

对所有点进行v-1次松弛操作 就可以找到从原点到所有点的最短路径

一次松弛操作 一个点到另一个的距离 和 一个点经过一个点再到另一个点的距离 比较

v-1次松弛操作 一个点到另一个的距离 和 一个点经过v-1个点再到另一个点的距离 比较

之后进行第v次松弛操作，如果还可更新距离，说明有负权环

 O(EV) 可以用队列优化 queue based ballman ford
```c++
...
拓扑排序 解决有向无环图的单源最短路径问题  O(v+e)

floyed算法解决无负权环的所有对最短路径算法 O(v^3)
